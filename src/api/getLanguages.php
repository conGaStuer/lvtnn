...
if (isset($_GET['languagesId'])) {
    ...
    $sql_get_languagesId = "SELECT s.maSach AS MaSach, s.chiTiet AS ChiTiet, nxbs.tenNXB AS NhaXuatBan,
   ...
   WHERE nn.maNN = '$languagesId'";
    $books = [];

    $result = $conn->query($sql_get_languagesId);
    if ($result->num_rows > 0) {
        ...
        json_encode(['Error' => "Invalid Book publisher"]);
