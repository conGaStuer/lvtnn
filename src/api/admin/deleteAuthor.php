...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Check if 'maTG' parameter is set in the POST request
    if (isset($request['maTG'])) {
        ...
        try {
            ...
            $stmt1 = $conn->prepare("DELETE FROM tg_sach WHERE maTG = ?");
            $stmt1->bind_param("i", $maTG);
            $stmt1->execute();

            // Prepare the SQL statement for deleting the category
            $stmt2 = $conn->prepare("DELETE FROM tac_gia WHERE maTG = ?");
            $stmt2->bind_param("i", $maTG);
            $stmt2->execute();
        ...
        // Respond with an error message if 'maTG' parameter is missing
        echo json_encode(["status" => "error", "message" => "'maTG' parameter is required."]);
    }
} else {
