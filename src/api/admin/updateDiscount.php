<?php
include '../config.php'; // Update with your actual database connection script

$input = json_decode(file_get_contents("php://input"), true);

if (isset($input['MaKhuyenMai']) && isset($input['TenKhuyenMai'])) {
    $MaKhuyenMai = $input['MaKhuyenMai'];
    $TenKhuyenMai = $input['TenKhuyenMai'];

    $query = "UPDATE khuyen_mai SET luongKM = ? WHERE maKM = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("si", $TenKhuyenMai, $MaKhuyenMai);

    if ($stmt->execute()) {
        echo json_encode("Cap Nhat Thanh Cong");
    } else {
        echo json_encode("Cap Nhat Khong Thanh Cong");
    }

    $stmt->close();
    $conn->close();
} else {
    echo json_encode("Du Lieu Khong Hop Le");
}
?>