...
export default {
  ...
  setup() {
    ...
    const placeOrder = (paymentMethod) => {
      ...
      const allAvailable = cartItems.value.every((item) => {
        const availableQuantity = bookMap.get(parseInt(item.MaSach)) || 0;
        console.log(
          `Checking item: ${item.MaSach}, Cart quantity: ${item.SoLuong}, Available quantity: ${availableQuantity}`
        );
