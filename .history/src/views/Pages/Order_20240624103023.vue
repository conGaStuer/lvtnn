          ...
          <div class="order-items">
            ...
            >
              ...
              <div class="product-details">
                ...
                <h4>{{ item.DanhMuc.join(", ") }}</h4>
                <p>
                  Tác giả: <span>{{ item.TacGia.join(", ") }}</span>
                </p>
                <p>
                  Nhà xuất bản: <span>{{ item.NhaXuatBan }}</span>
                </p>
                <p>
                  Ngôn ngữ: <span>{{ item.NgonNgu.join(", ") }}</span>
                </p>
              </div>
              <div class="product-price">
                <p>{{ calculateDiscountedPrice(item) }}</p>
              </div>
...
const parseOrderItems = (order) => {
  return order.TacGia.map((tacGia, index) => {
    return {
      MaSach: order.MaSach[index],
      TenSach: order.TenSach[index],
      HinhAnh: order.HinhAnh[index],
      DonGia: order.DonGia[index],
      SoLuong: order.SoLuong[index],
      TacGia: tacGia,
      NgonNgu: order.NgonNgu[index],
      DanhMuc: order.DanhMuc[index],
      KhuyenMai: order.KhuyenMai[index],
    };
  });
};
...
const calculateDiscountedPrice = (item) => {
  return (item.DonGia - (item.DonGia * item.KhuyenMai) / 100).toFixed(2);
};
