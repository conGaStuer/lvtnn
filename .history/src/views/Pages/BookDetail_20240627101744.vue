...
export default {
  ...
  setup() {
    ...
    const handleComment = (repMaDG = null) => {
      axios
        .post("http://localhost/LVTN/book-store/src/api/postComment.php", {
          ...
          rating: value.value,
          repMaDG: repMaDG, // Gửi kèm mã bình luận được trả lời
        })
        .then((res) => {
          if (res.data === "Danh gia thanh cong") {
            getAllComment();
            alert("Thêm thành công");
            userComment.value = ""; // Xóa nội dung bình luận sau khi gửi thành công
            value.value = null; // Reset giá trị đánh giá sau khi gửi thành công
            isRep.value = null; // Đặt lại trạng thái trả lời
          } else {
            console.log("Sai");
          }
        })
