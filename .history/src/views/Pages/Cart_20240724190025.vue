...
export default {
  ...
  setup() {
    ...
    const placeOrder = (paymentMethod) => {
      ...
      const allAvai = cartItems.value.every((item) => {
        const avaiQuantity = bookMap.get(parseInt(item.MaSach)) || 0;
        console.log(
          `Checking item: ${item.MaSach}, Cart quantity: ${item.SoLuong}, Available quantity: ${avaiQuantity}`
        );
        return parseInt(item.SoLuong) <= avaiQuantity;
      });

      if (!allAvai) {
        alert(
          "Sản phẩm này trong giỏ hàng đã hết hàng, vui lòng xóa khỏi giỏ hàng!"
        );
        return;
      }
