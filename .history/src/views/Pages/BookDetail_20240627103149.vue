...
export default {
  ...
  setup() {
    ...
    const handleComment = (rep = null) => {
      ...
      axios
        .post(
          `http://localhost/LVTN/book-store/src/api/postComment.php?MaSach=${route.params.id}&MaKH=${userId}`,
          {
            noidung: userComment,
            rating: ratingValue,
            rep: rep,
    ...
    const repComment = (commentId) => {
      if (
        currentUser.vaiTro === "nhanvien" ||
        currentUser.vaiTro === "khachang"
      ) {
    ...
    const isEmployee = computed(() => {
      return currentUser.vaiTro === "nhanvien";
    });

    const isEmployeeOrCustomer = computed(() => {
      return (
        currentUser.vaiTro === "nhanvien" || currentUser.vaiTro === "khachang"
      );
    });
