<template>
  <div v-if="popUp" class="popUp">
    <div class="pop">
      ...
      <p>
        Vui lòng cập nhật đầy đủ tên, số điện thoại, email, địa chỉ trong phần
        hồ sơ để đơn hàng được duyệt
      </p>
    </div>
  </div>
  <NavBar></NavBar>
  ...
  setup() {
    ...
    const getAllBook = () => {
      axios
        ...
        .then((response) => {
          if (response.data) {
            bookss.value = response.data;
            console.log(bookss.value);
          } else {
            console.log("Loi");
          }
        })
    ...
    const selectAllCheck = () => {
      if (selectAll.value) {
        selectedItems.value = cartItems.value.map((item) => item.MaSach);
      } else {
        selectedItems.value = "";
      }
    };
    const placeOrder = (paymentMethod) => {
      ...
      const bookMap = new Map(
        bookss.value.map((book) => [
          parseInt(book.MaSach),
          parseInt(book.SoLuong),
        ])
      );

      // Log để kiểm tra dữ liệu bookMap
      console.log("bookMap:", bookMap);

      // Kiểm tra số lượng hàng
      const allAvailable = selectedItems.value.every((maSach) => {
        const item = cartItems.value.find((item) => item.MaSach === maSach);
        if (item) {
          const availableQuantity = bookMap.get(parseInt(maSach)) || 0;
          console.log(
            `Checking item: ${maSach}, Cart quantity: ${item.SoLuong}, Available quantity: ${availableQuantity}`
          );
          return parseInt(item.SoLuong) <= availableQuantity;
        }
        return false;
      });
      ...
      if (!allAvailable) {
        alert(
          "Có sản phẩm trong giỏ hàng đã hết hàng, vui lòng xóa sản phẩm này khỏi giỏ hàng!"
        );
        return;
      }
