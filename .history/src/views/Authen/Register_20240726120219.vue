...
export default {
  setup() {
    ...
    watch([password, confirmPassword], checkPasswords);
    const handleUsername = () => {
      const spaceOrDiacriticalRegex =
      ...
      if (username.value.length >= 15) {
        ...
        username.value = "";
      } else if (spaceOrDiacriticalRegex.test(username.value)) {
        alert(
          "Tên đăng nhập không được chứa khoảng trắng hoặc ký tự tiếng Việt có dấu"
        );
        username.value = "";
      }
    };
