<template>
  <div class="stick">
    ...
    <df-messenger
      intent="WELCOME"
      chat-title="Bookapeeeeee"
      agent-id="bf284466-2643-409a-bc0f-8fa2b8800c78"
      language-code="en"
      class="chatbot"
    ></df-messenger>
    ...
    <div v-else-if="searchResults.length === 0 && searchKeyWord.length > 0">
...
export default {
  setup() {
    ...
    const searchProduct = () => {
      axios
        .get(
          `http://localhost/LVTN/book-store/src/api/searchProduct.php?keyword=${searchKeyWord.value}`
        )
        .then((response) => {
          if (response.data) {
