  ...
  <div class="order-container">
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <span v-else-if="column.key === 'HinhAnh'">
          <div
            v-for="(hinhAnh, index) in record.HinhAnh"
            :key="index"
        ...
        <span v-else-if="column.key === 'SoLuong'">
          <ul>
            <span v-for="(soLuong, index) in record.SoLuong" :key="index">
              {{ soLuong }}
            </span>
          </ul>
        </span>
        <span v-else-if="column.key === 'DonGia'">
          <ul>
            <span v-for="(donGia, index) in record.DonGia" :key="index">
              {{ donGia }}
            </span>
          </ul>
        </span>
    ...
    >
      <div v-if="selectedOrder">
        ...
        <ul>
          <div
            v-for="(tenSach, index) in selectedOrder.TenSach"
            :key="index"
            class="detail"
          >
            <img
              :src="selectedOrder.HinhAnh[index]"
              alt="product-image"
              style="width: 80px; height: 100px"
            />
            <div class="detaill">
              <span><strong>Tên Sách:</strong> {{ tenSach }}</span>
              <span>
                <strong>Số Lượng:</strong> {{ selectedOrder.SoLuong[index] }}
              </span>
              <span
                ><strong>Đơn Giá:</strong>
                {{ selectedOrder.DonGia[index] }}</span
              >
            </div>
          </div>
        </ul>
...
const columns = ref([
  ...
  { title: "Tên Sách", dataIndex: "TenKH", key: "TenKH" },
