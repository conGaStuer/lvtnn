      ...
      <div class="topPro">
        ...
        <div class="tops">
          <div class="top" v-for="product in topProducts" :key="product.id">
            ...
            <div><b>Đã bán : </b>{{ product.unitsSold }} sản phẩm</div>
            <div><b>Số lượng còn lại trong kho :</b> {{ product.SoLuong }}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    >
      <div v-if="modalType === 'orders'">
        <ul>
          <li v-for="order in stats.orders" :key="order.madon">
            <b>Mã đơn: </b> <i>{{ order.madon }}</i> - <b>Ngày đặt: </b>
            <i>{{ order.ngaydat }}</i>
          </li>
        </ul>
      </div>
      <div v-else-if="modalType === 'revenue'">
        <ul>
          <li v-for="order1 in stats.revenue_details" :key="order1.madon">
            <b>Mã đơn: </b> <i>{{ order1.madon }}</i> - <b>Ngày đặt: </b>
            <i>{{ order1.ngaydat }}</i> - <b> Tổng doanh thu :</b>
      ...
      <div v-else-if="modalType === 'users'">
        <ul>
          <li v-for="order in stats.orders" :key="order.madon">
            <b>Mã đơn: </b> <i>{{ order.madon }}</i> - <b>Ngày đặt: </b>
            <i>{{ order.ngaydat }}</i> - <b>Mã người dùng :</b>
            <i>{{ order.maND }}</i>
          </li>
        </ul>
      </div>
      <div v-else-if="modalType === 'quantity'">
        <ul>
          <li v-for="order2 in stats.quantity_details" :key="order2.madon">
            <b>Mã đơn: </b> <i>{{ order2.madon }}</i> - <b>Ngày đặt: </b>
            <i>{{ order2.ngaydat }}</i> - <b> Số lượng :</b>
