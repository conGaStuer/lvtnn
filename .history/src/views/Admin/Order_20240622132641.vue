  ...
  <div class="order-container">
    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        ...
        <span v-else-if="column.key === 'MaSach'">
          <template v-for="(maSach, index) in record.MaSach" :key="index">
            <span v-if="index > 0">, </span>
            {{ maSach }}
          </template>
        </span>
        <span v-else-if="column.key === 'TenSach'">
          <template v-for="(tenSach, index) in record.TenSach" :key="index">
            <span v-if="index > 0">, </span>
            {{ tenSach }}
          </template>
        </span>
        <span v-else-if="column.key === 'HinhAnh'">
          <template v-for="(hinhAnh, index) in record.HinhAnh" :key="index">
            <img
              :src="hinhAnh"
              alt="product-image"
        ...
        <span v-else-if="column.key === 'TacGia'">
          <template v-for="(tacGia, index) in record.TacGia" :key="index">
            <span v-if="index > 0">, </span>
            {{ tacGia }}
          </template>
        </span>
        <span v-else-if="column.key === 'NgonNgu'">
          <template v-for="(ngonNgu, index) in record.NgonNgu" :key="index">
            <span v-if="index > 0">, </span>
            {{ ngonNgu }}
          </template>
        </span>
        <span v-else-if="column.key === 'DanhMuc'">
          <template v-for="(danhMuc, index) in record.DanhMuc" :key="index">
            <span v-if="index > 0">, </span>
            {{ danhMuc }}
          </template>
        </span>
        <span v-else-if="column.key === 'SoLuong'">
          <template v-for="(soLuong, index) in record.SoLuong" :key="index">
            <span v-if="index > 0">, </span>
            {{ soLuong }}
          </template>
        </span>
        <span v-else-if="column.key === 'DonGia'">
          <template v-for="(donGia, index) in record.DonGia" :key="index">
            <span v-if="index > 0">, </span>
            {{ donGia }}
          </template>
        </span>
        <span v-else-if="column.key === 'GiaDonHang'">
          <template
            v-for="(giaDonHang, index) in record.GiaDonHang"
            :key="index"
          >
            <span v-if="index > 0">, </span>
            {{ giaDonHang }}
...
const orders = ref([]);
const columns = ref([
  { title: "Mã Đơn", dataIndex: "MaDon", key: "MaDon" }, // Thêm cột Mã Đơn vào đây
  ...
  { title: "Giá Đơn Hàng", dataIndex: "GiaDonHang", key: "GiaDonHang" },
