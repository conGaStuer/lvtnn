  ...
  <div class="order-container">
    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        ...
        <span v-if="column.key === 'MaSach'">
          <template v-for="(item, index) in record.MaSach" :key="index">
        ...
        <span v-if="column.key === 'TenSach'">
          <template v-for="(item, index) in record.TenSach" :key="index">
            <span v-if="index > 0">, </span>
            {{ item }}
          </template>
        </span>

        <span v-if="column.key === 'HinhAnh'">
          <template v-for="(item, index) in record.HinhAnh" :key="index">
            <img
              :src="item"
              alt="product-image"
        ...
        <span v-if="column.key === 'TacGia'">{{ record.TacGia }}</span>
        <span v-if="column.key === 'NgonNgu'">{{ record.NgonNgu }}</span>
        <span v-if="column.key === 'DanhMuc'">{{ record.DanhMuc }}</span>
        <span v-if="column.key === 'SoLuong'">{{ record.SoLuong }}</span>
        <span v-if="column.key === 'DonGia'">{{ record.DonGia }}</span>
        <span v-if="column.key === 'KhuyenMai'">{{ record.KhuyenMai }}</span>
        <span v-if="column.key === 'TrangThai'">{{
          displayStatus(record.TrangThai)
        }}</span>
        <span v-if="column.key === 'MaND'">{{ record.MaND }}</span>
