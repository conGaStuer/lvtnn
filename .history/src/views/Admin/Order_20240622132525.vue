    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        ...
        <span v-else-if="column.key === 'MaSach'">
          <template v-for="(maSach, index) in record.MaSach">
            <span v-if="index > 0">, </span>
            {{ maSach }}
          </template>
        </span>
        <span v-else-if="column.key === 'TenSach'">
          <template v-for="(tenSach, index) in record.TenSach">
            <span v-if="index > 0">, </span>
            {{ tenSach }}
          </template>
        </span>
        <span v-else-if="column.key === 'HinhAnh'">
          <template v-for="(hinhAnh, index) in record.HinhAnh">
            <img
              :key="index"
              :src="hinhAnh"
              alt="product-image"
        ...
        <span v-else-if="column.key === 'TacGia'">
          <template v-for="(tacGia, index) in record.TacGia">
            <span v-if="index > 0">, </span>
            {{ tacGia }}
          </template>
        </span>
        <span v-else-if="column.key === 'NgonNgu'">
          <template v-for="(ngonNgu, index) in record.NgonNgu">
            <span v-if="index > 0">, </span>
            {{ ngonNgu }}
          </template>
        </span>
        <span v-else-if="column.key === 'DanhMuc'">
          <template v-for="(danhMuc, index) in record.DanhMuc">
            <span v-if="index > 0">, </span>
            {{ danhMuc }}
          </template>
        </span>
        <span v-else-if="column.key === 'SoLuong'">
          <template v-for="(soLuong, index) in record.SoLuong">
            <span v-if="index > 0">, </span>
            {{ soLuong }}
          </template>
        </span>
        <span v-else-if="column.key === 'DonGia'">
          <template v-for="(donGia, index) in record.DonGia">
            <span v-if="index > 0">, </span>
            {{ donGia }}
          </template>
        </span>
        <span v-else-if="column.key === 'GiaDonHang'">
          <template v-for="(giaDonHang, index) in record.GiaDonHang">
            <span v-if="index > 0">, </span>
            {{ giaDonHang }}
          </template>
        </span>
