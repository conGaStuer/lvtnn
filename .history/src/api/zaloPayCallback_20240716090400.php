...
try {
    ...
    // Create a new order
    $stmt_create_order = $conn->prepare("INSERT INTO don_dat_hang (maND, trangthai, ngaydat) VALUES (?, 'daduyet', CURDATE())");
    $stmt_create_order->bind_param('i', $userId);
    if ($stmt_create_order->execute()) {
        $orderId = $conn->insert_id;

        // Add event
        $stmt_add_event = $conn->prepare("INSERT INTO order_events (order_id, event, timestamp) VALUES (?, 'daduyet', NOW())");
