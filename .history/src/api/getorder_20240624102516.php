...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        // Convert các thuộc tính có thể có nhiều giá trị thành mảng
        $order = [
            'MaDon' => $row['MaDon'],
            ...
            'KhuyenMai' => explode(",", $row['KhuyenMai']),
            // Lấy một giá trị duy nhất từ Group_CONCAT, chẳng hạn như lấy giá trị đầu tiên
            'MaSach' => $row['MaSach'],
            'TenSach' => $row['TenSach'],
            'HinhAnh' => $row['HinhAnh'],
        ];
