...
$sql_orders = "SELECT COUNT(*) as total_orders, GROUP_CONCAT(madon) as madon_list, GROUP_CONCAT(ngaydat) as ngaydat_list 
...
if ($result_orders->num_rows > 0) {
    ...
    $madonList = explode(",", $row_orders['madon_list']);
    $ngaydatList = explode(",", $row_orders['ngaydat_list']);

    for ($i = 0; $i < count($madonList); $i++) {
        $response['orders'][] = array(
            'madon' => $madonList[$i],
            'ngaydat' => $ngaydatList[$i],
        );
    }
} else {
