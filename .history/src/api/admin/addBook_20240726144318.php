...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_check_book_name = "SELECT * FROM sach WHERE maSach = '$tenSach'";
    $result_book_name = $conn->query($sql_check_book_name);

    if ($result_book_name->num_rows > 0) {
        $sql_update_book_quantity = "UPDATE sach SET soLuong = soLuong + $soLuong WHERE tenSach = '$tenSach'";
        if ($conn->query($sql_update_book_quantity) === true) {
            echo json_encode("Them Thanh Cong");
        } else {
            echo json_encode("Cap nhat so luong that bai.");
        }
    } else {
