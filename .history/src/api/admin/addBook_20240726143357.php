...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result_book_name->num_rows > 0) {
        $sql_insert_new_book1 = "UPDATE sach SET soLuong = soLuong + $soLuong where tenSach = '$tenSach'";
        $conn->query($sql_insert_new_book1);
        if ($conn->query($sql_update_book_quantity) === true) {
            echo json_encode("Them thanh cong");
        } else {
            echo json_encode("Cập nhật số lượng sách thất bại.");
        }
    } else {
