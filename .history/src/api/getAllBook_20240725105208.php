...
$sql_get = "SELECT s.maSach AS MaSach, s.chiTiet AS ChiTiet,
       ...
       s.soLuong as SoLuong, km.luongKM AS KhuyenMai,
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $row['TacGia'] = array_unique(explode(',', $row['TacGia']));
        $row['NgonNgu'] = array_unique(explode(',', $row['NgonNgu']));
        $row['DanhMuc'] = array_unique(explode(',', $row['DanhMuc']));
