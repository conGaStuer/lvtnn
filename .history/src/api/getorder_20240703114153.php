...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $books = [];
        foreach ($row['MaSach'] as $index => $maSach) {
            if (!isset($books[$maSach])) {
                $books[$maSach] = [
                    'MaSach' => $maSach,
                    'TenSach' => $row['TenSach'][$index],
                    'HinhAnh' => $row['HinhAnh'][$index],
                    'DonGia' => $row['DonGia'][$index],
                    'GiaDonHang' => $row['GiaDonHang'][$index],
                    'SoLuong' => $row['SoLuong'][$index],
                    'TacGia' => $row['TacGia'][$index],
                    'NgonNgu' => $row['NgonNgu'][$index],
                    'KhuyenMai' => $row['KhuyenMai'][$index],
                    'NhaXuatBan' => $row['NhaXuatBan'][$index],
                    'DanhMuc' => []
                ];
            }
            $books[$maSach]['DanhMuc'][] = $row['DanhMuc'][$index];
        }

        // Convert the aggregated book data back to the order format
        foreach ($books as &$book) {
            $book['DanhMuc'] = implode(', ', array_unique($book['DanhMuc']));
        }

        $row['Books'] = array_values($books);
        $orders[] = $row;
    }
