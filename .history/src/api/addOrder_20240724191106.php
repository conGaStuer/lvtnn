...
try {
    ...
    if ($conn->query($sql_create_order) === TRUE) {
        ...
        foreach ($items as $item) {
            ...
            $sql_delete_item = "DELETE FROM chi_tiet_don_hang WHERE madon IN (SELECT madon FROM don_dat_hang WHERE maND = '$userId' AND trangthai = 'giohang') AND masach = '$maSach'";
            if ($conn->query($sql_delete_item) !== TRUE) {
                throw new Exception("Failed to delete item from cart: " . $conn->error);
            }
        }
