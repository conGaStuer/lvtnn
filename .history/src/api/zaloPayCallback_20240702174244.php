...
try {
    ...
    if (strcmp($mac, $requestmac) !== 0) {
        ...
        $status = $datajson["status"]; // Payment status

        // Log received data for debugging
        file_put_contents("logs/zalopay_log.txt", print_r($datajson, true), FILE_APPEND);
        ...
        if ($status == 1) { // Payment successful
            ...
            $result["return_code"] = 0;
            $result["return_message"] = "Payment not successful, status: " . $status;
        }
    }
} catch (Exception $e) {
    $result["return_code"] = 0;
