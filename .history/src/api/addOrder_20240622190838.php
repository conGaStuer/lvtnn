...
if ($conn->query($sql_create_order) === TRUE) {
    ...
    foreach ($items as $item) {
        $maSach = $item['MaSach'];
        $sql_delete_cart_item = "DELETE FROM chi_tiet_don_hang WHERE madon
         IN (SELECT madon FROM don_dat_hang WHERE maND = '$userId' AND trangthai = 'giohang') AND masach = '$maSach'";
    ...
    $sql_delete_empty_cart_orders = "DELETE FROM don_dat_hang WHERE maND = '$userId' AND trangthai = 'giohang'
     AND madon NOT IN (SELECT madon FROM chi_tiet_don_hang)";
