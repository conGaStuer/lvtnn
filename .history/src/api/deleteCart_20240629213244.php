...
if ($_SERVER["REQUEST_METHOD"] == 'POST') {
    ...
    if (isset($data->maSach)) {
        // First, get the `madon` and `trangthai` for the given `maSach`
        $stmt = $conn->prepare("SELECT ctdh.madon, ddh.trangthai FROM chi_tiet_don_hang ctdh
        ...
        if (count($orderIds) > 0) {
            ...
            echo json_encode(array("message" => "No matching orders found or the item is already ordered."));
        }
    } else {
