...
SELECT 
    ...
    GROUP_CONCAT(DISTINCT ctdh.masach ORDER BY ctdh.masach) AS MaSach, 
    GROUP_CONCAT(DISTINCT nxb.tenNXB ORDER BY ctdh.masach) AS NhaXuatBan,  
    GROUP_CONCAT(DISTINCT s.tenSach ORDER BY ctdh.masach) AS TenSach, 
    GROUP_CONCAT(DISTINCT s.hinhAnh ORDER BY ctdh.masach) AS HinhAnh,
    GROUP_CONCAT(DISTINCT s.donGia ORDER BY ctdh.masach) AS DonGia, 
    GROUP_CONCAT(DISTINCT ctdh.SoLuong ORDER BY ctdh.masach) AS SoLuong, 
    SUM(ctdh.DonGia * ctdh.SoLuong) AS GiaDonHang,
    GROUP_CONCAT(DISTINCT tg.tenTG ORDER BY ctdh.masach) AS TacGia, 
    GROUP_CONCAT(DISTINCT nn.tenNN ORDER BY ctdh.masach) AS NgonNgu, 
    GROUP_CONCAT(DISTINCT dm.tenDM ORDER BY ctdh.masach) AS DanhMuc,
    GROUP_CONCAT(DISTINCT km.luongKM ORDER BY ctdh.masach) AS KhuyenMai
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $row['DonGia'] = array_map('intval', $row['DonGia']); // Convert DonGia to integers
